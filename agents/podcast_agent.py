# ##################################### ------ {YT with Links Integrated - Live Agent} ------#####################################
# import os
# import asyncio
# import logging
...
#         def fetch_podcast_episodes(state: PodcastState) -> PodcastState:
#             logger.info(f"Fetching podcast episodes for: {state['podcast_name']}")
#             episodes = [
#                 {
#                     'title': f"Episode 1 - Guest A on {state['podcast_name']}",
#                     'guest': 'Guest A',
#                     'date': (datetime.now() - timedelta(days=1)).isoformat(),
#                     'duration': '60 minutes'
#                 },
#                 {
#                     'title': f"Episode 2 - Guest B on {state['podcast_name']}",
#                     'guest': 'Guest B', 
...
#             logger.info(f"Fetched {len(episodes)} episodes for {state['podcast_name']}.")
#             return state

#         def summarize_episodes(state: PodcastState) -> PodcastState:
#             logger.info(f"Starting summarization for {len(state['episodes'])} episodes.")
#             summaries = []
#             for episode in state['episodes']:
#                 logger.info(f"Summarizing episode: {episode['title']} with guest {episode['guest']}.")
#                 summary = {
#                     'episode_title': episode['title'],
#                     'guest': episode['guest'],
#                     'bullets': [
#                         f"Key insight {i+1} from {episode['guest']} discussing market trends and investment strategies"
#                         for i in range(10)
#                     ]
#                 }
#                 summaries.append(summary)
#                 logger.info(f"Completed summarization for episode: {episode['title']}.")
#             state['summaries'] = summaries
#             logger.info("Summarization complete for all episodes.")
#             return state

#         def tsi_integration_analysis(state: PodcastState) -> PodcastState:
#             logger.info("Starting TSI integration analysis.")
#             tsi_prompt = f"""
#             Analyze the following podcast insights and integrate them with our TSI framework:
            
#             {self.tsi_model}
            
#             Summarize key insights by topic (macro, precious metals, etc.) in 100 bullets maximum.
#             Add expert names in brackets after each insight.
#             Provide investment recommendations for 1-6 months based on insights + TSI model + astrology.
            
#             Podcast Insights: {json.dumps(state['summaries'], indent=2)}
#             """
#             # Currently mocked output; integrate LLM later if needed
#             state['tsi_analysis'] = "TSI-integrated analysis with investment recommendations"
#             logger.info("Completed TSI integration analysis.")
#             return state

#         def compile_podcast_output(state: PodcastState) -> PodcastState:
#             logger.info("Compiling final podcast output.")
#             output = f"PODCAST ANALYSIS: {state['podcast_name']}\n" + "="*60 + "\n\n"
#             output += "EPISODE SUMMARIES:\n" + "-"*30 + "\n\n"
#             for summary in state['summaries']:
#                 output += f"**{summary['episode_title']}**\n"
#                 output += f"Guest: {summary['guest']}\n\n"
#                 for i, bullet in enumerate(summary['bullets'], 1):
#                     output += f"{i}. {bullet}\n"
#                 output += "\n"
#             output += "\nTSI INTEGRATION ANALYSIS:\n" + "-"*40 + "\n"
#             output += state['tsi_analysis']
#             state['final_output'] = output
#             logger.info("Final podcast output compilation complete.")
#             return state

#         workflow = StateGraph(PodcastState)
#         workflow.add_node("fetch_episodes", fetch_podcast_episodes)
#         workflow.add_node("summarize_episodes", summarize_episodes)
#         workflow.add_node("tsi_analysis", tsi_integration_analysis)
#         workflow.add_node("compile_output", compile_podcast_output)
#         workflow.add_edge("fetch_episodes", "summarize_episodes")
#         workflow.add_edge("summarize_episodes", "tsi_analysis")
#         workflow.add_edge("tsi_analysis", "compile_output")
#         workflow.add_edge("compile_output", END)
#         workflow.set_entry_point("fetch_episodes")
        
#         return workflow.compile()

#     async def run(self, podcast_name: str) -> str:
#         logger.info(f"\t X<----> Starting PodcastAgent run for: {podcast_name}. <---->X\n")
...
#         logger.info(f"\t X<----> PodcastAgent run complete for: {podcast_name}. <---->X\n")
#         return result['final_output']

# # Example usage:
# # if __name__ == "__main__":
# #     agent = PodcastAgent(llm_api_key="YOUR_DEEPSEEK_API_KEY")
# #     output = asyncio.run(agent.run("Macro Voices"))
# #     print(output)
